...
DIRECTIONS_TABLE = {DIRECTION_NORTH, DIRECTION_EAST, DIRECTION_SOUTH, DIRECTION_WEST, DIRECTION_SOUTHWEST,
...
function addStamina(playerId, ...)
        ...
        if configManager.getBoolean(configKeys.STAMINA_TRAINER) then
            if not player then
                ...
                if not target or target:getName() ~= staminaBonus.target then
                    ...
                    player:sendTextMessage(MESSAGE_FAILURE, string.format("%i of stamina has been refilled.",
    ...
    if localPlayerId and delay then
        ...
        player:sendTextMessage(MESSAGE_FAILURE, string.format("%i minute%s of stamina has been refilled.", regen,
            regen == 1 and "" or "s"))
