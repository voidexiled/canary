...
function DatabaseRoulettePlays:create(uuid, playerId, reward)
    db.query(('INSERT INTO roulette_plays (player_id, uuid, reward_id, reward_count) VALUES (%d, %s, %d, %d)'):format(
        playerId, db.escapeString(uuid), reward.id, reward.count))
end

function DatabaseRoulettePlays:update(uuid, status)
    db.query(('UPDATE roulette_plays SET status = %d, updated_at = %d WHERE uuid = %s'):format(status, os.time(),
        db.escapeString(uuid)))
end

function DatabaseRoulettePlays:select(uuid)
    local resultId = db.storeQuery(
        ('SELECT player_id, reward_id, reward_count FROM roulette_plays WHERE uuid = %s'):format(db.escapeString(uuid)))
...
function DatabaseRoulettePlays:selectPendingPlayRewardsByPlayerGuid(guid)
    ...
    local resultId = db.storeQuery(
        ('SELECT uuid, reward_id, reward_count FROM roulette_plays WHERE player_id = %d AND status = 1'):format(guid))
