local config_aura = {
    effect = 260, -- efeito que vai ficar girando no player
...
function aura.onSay(player, words, param)
    ...
    if param == "on" then
        if (player:getLevel() > config_aura.level_use.max) then
            return (not (player:sendTextMessage(MESSAGE_STATUS_SMALL, player:getName() .. " seu level(" ..
                player:getLevel() .. ") precisa ser maior(" .. config_aura.level_use.max)))
        end

        if (player:getLevel() < config_aura.level_use.min) then
            return (not (player:sendTextMessage(MESSAGE_STATUS_SMALL, player:getName() .. " seu level(" ..
                player:getLevel() .. ") precisa ser menor(" .. config_aura.level_use.max)))
        end

        if (player:getStorageValue(config_aura.storage) >= 1) then
            return (not (player:sendTextMessage(MESSAGE_STATUS_SMALL,
                player:getName() .. " Você ja esta com a aurea ligada")))
        end
